...
import { Sparkles, Check, Phone, MessageCircle, Star } from 'lucide-react';
          ...
          <div className="max-w-6xl mx-auto">
            <header className="text-center mb-12">
              ...
              <p className="suz-text-body-lg text-slate-400 max-w-2xl mx-auto">
                Systematische Bodenreinigung in vier professionellen Schritten
              </p>
            </header>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {serviceData.process.map((step, index) => (
                <div
                  key={index}
                  className="suz-card-glass rounded-xl border border-white/10 text-center group hover:border-purple-400/30 transition-all duration-300"
                  role="article"
                  aria-label={`Schritt ${step.step}: ${step.title}`}
                >
                  <div className="p-6">
                    <div className="suz-icon-badge-premium mb-4 mx-auto bg-purple-500/20 border-purple-400/30">
                      <span className="text-2xl font-bold text-purple-300 group-hover:text-purple-200 transition-colors duration-300">
                        {step.step}
                      </span>
                    </div>
                    <h3 className="suz-text-heading-sm font-semibold text-slate-100 mb-3 group-hover:text-purple-200 transition-colors duration-300">
                      {step.title}
                    </h3>
        ...
        >
          <div className="max-w-6xl mx-auto">
            <header className="text-center mb-12">
              ...
              <p className="suz-text-body-lg text-slate-400 max-w-2xl mx-auto">
                Transparente Preise für professionelle Bodenreinigung
              </p>
            </header>
            <div className="grid md:grid-cols-3 gap-8">
              {serviceData.pricing.options.map((option, index) => (
                <div
                  key={index}
                  className={`suz-card-glass rounded-2xl border transition-all duration-300 group hover:border-purple-400/40 hover:shadow-2xl ${
                    index === 1
                ...
                >
                  <div className="p-8">
                    <header className="text-center mb-6">
                      <h3 className="suz-text-heading-md font-semibold text-slate-100 mb-3 group-hover:text-purple-200 transition-colors duration-300">
                        {option.name}
                      </h3>
                      <div className="suz-text-display-sm font-bold text-purple-400 mb-2 group-hover:text-purple-300 transition-colors duration-300">
                        {option.price}
                      </div>
                      <p className="suz-text-body-sm text-slate-400 group-hover:text-slate-300 transition-colors duration-300">
                        {option.description}
                    ...
                    <ul className="space-y-3" role="list" aria-label="Leistungen">
                      {option.features.map((feature, featureIndex) => (
                        ...
                        >
                          <Check
                            className="w-5 h-5 text-purple-400 mr-3 flex-shrink-0 group-hover:text-purple-300 transition-colors duration-300"
                            aria-hidden="true"
                          />
                          {feature}
                        </li>
        ...
        >
          <div className="max-w-4xl mx-auto">
            <header className="text-center mb-12">
              ...
              <p className="suz-text-body-lg text-slate-400 max-w-2xl mx-auto">
                Antworten auf die wichtigsten Fragen zur Bodenreinigung
              </p>
            </header>
            <div className="space-y-6" role="list" aria-label="FAQ Liste">
              {serviceData.faqs.map((faq, index) => (
                <article
                  key={index}
                  className="suz-card-glass rounded-xl border border-white/20 group hover:border-purple-400/30 transition-all duration-300"
                  role="listitem"
                >
                  <div className="p-6">
                    <h3 className="suz-text-heading-sm font-semibold text-slate-100 mb-3 group-hover:text-purple-200 transition-colors duration-300">
